...
const VerifyOfferLetters = () => {
  ...
  const downloadOfferLetter = async (studentId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/admin/download-offer-letter/${studentId}`, {
        method: "GET",
  ...
  const downloadEmployeeProof = async (studentId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/admin/download-employee-proof/${studentId}`, {
        method: "GET",
  ...
  const handleVerify = async (studentId) => {
    try {
      await axios.put(`http://localhost:5000/api/admin/verify-student/${studentId}`);
