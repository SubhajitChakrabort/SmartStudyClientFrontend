...
const TeacherStudyFolder = () => {
  ...
  const fetchFolderNotes = async (folderId) => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/study-folders/${folderId}/notes`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchAvailableNotes = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/teacher-uploads', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleDeleteFolder = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/study-folders/${folderToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
