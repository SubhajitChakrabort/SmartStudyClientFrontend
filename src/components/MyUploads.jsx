...
const MyUploads = () => {
  ...
  const fetchUploads = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/upload/user', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/upload/${currentUpload.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleDeleteConfirm = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/upload/${deleteId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const renderFilePreview = (upload) => {
    const fileUrl = `http://localhost:5000${upload.file_path}`;
