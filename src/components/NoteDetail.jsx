  ...
  useEffect(() => {
    ...
    const fetchNoteAndSummary = async () => {
      try {
        ...
        // Fetch note details
        const noteResponse = await fetch(`http://localhost:5000/api/notes/${noteId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        
        // Fetch summary
        const summaryResponse = await fetch(`http://localhost:5000/api/note-summaries/summaries?note_id=${noteId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
